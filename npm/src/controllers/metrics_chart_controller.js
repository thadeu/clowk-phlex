 ...
 if (v > max) max = v })
  ...
  applySeriesStyles(highlightIdx = null) {
    ...
    this.multiSeries.forEach((s, idx) => {
      ...
      // Bars multi: the grouped columns dim/hide with their legend entry.
      this.barsForIndex(idx).forEach((b) => { b.style.opacity = markOpacity })
  ...
  barsForIndex(idx) {
    return this.barTargets.filter((b) => parseInt(b.dataset.seriesIndex, 10) === idx)
  }

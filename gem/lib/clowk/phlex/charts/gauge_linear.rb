...
# to stack one labeled capacity bar per row (e.g. one per pod), each with its own
# fill + amber/red threshold tint. The single-gauge args are ignored then.
class Clowk::Phlex::Charts::GaugeLinear < Clowk::Phlex::Component
  def initialize(pct: nil, color: "#5B8DEF", value_label: nil, capacity_label: nil, percent: true, center_value: nil, bars: nil)
  ...
  # multi_template — N capacity bars stacked vertically, one row per entry.
  def multi_template
    div(class: "flex flex-col justify-center gap-3.5 py-3 min-h-[120px]") do
  ...
  def gauge_row(bar)
    ...
    color = row_fill_color(pct, bar[:color] || @color)

    div(class: "flex flex-col gap-1.5") do
      div(class: "flex items-baseline justify-between gap-2") do
        span(class: "font-clowk-mono text-[11.5px] text-clowk-text-2 truncate min-w-0") { bar[:label].to_s }
      ...
      div(class: "h-2.5 w-full bg-clowk-surface-3 overflow-hidden rounded-clowk-sm") do
        div(style: "width: #{pct.round(1)}%; height: 100%; background: #{color};")
      end
